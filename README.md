...
Стоит сразу оговориться, что работа будет состоять из двух частей. В первой части мы напишем JIT компилятор, а во второй - полноценный бинарный транслятор(AOT) с записью в ELF (Executable and Linkable Format) файл.
