...
В прошлом семестре мы написали свой язык программирования. Написанный язык выполнялся на процессоре реализованном нами. В данной работе нам предстоит транслировать наш байт код в нативный код архитектуры x86-64.
...
Сперва, давайте углубимся в структуру команд нашего процессора. Первый байт, а именно первые 5 бит отвечают за ```cmd num```, за номер соответствующей команды (0-31), далее следует бит ```imm```, бит который показывает, оперирует ли команда со значениями, следующий бит - ```reg``` - будет ли присутствовать регистр, последний бит ```mem``` показывает, будут ли производится действия с оперативной памятью. Изобразим это схематично: 
